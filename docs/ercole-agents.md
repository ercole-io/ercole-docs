...
### Configuration
Ercole-agent can be configured modifying the content of the `config.json` stored in `/opt/ercole-agent/config.json` on Linux or `C:\ErcoleAgent\config.json` on Windows.
